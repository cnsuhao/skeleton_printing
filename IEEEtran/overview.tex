...
\emph{\textbf{Theorem 1}: Given three integer numbers $c$, $d$, and $m = min\{c,2^dn-c\}$, Let $S$ be a tree structure such that the degree of each node is bounded by $d$, then whether $S$ has $c$ support-free subgraphs can be determined in $O(2^{md}n^{2m})$ time, and a partition instance can be reported within the same time bound.}
