...
Therefore, let $b(H_i)$ be the base cut of the mesh part that corresponds to the root of $H_i$ \ca{(by base we mean the cutting cross section that touches the printing platform when the part is being printed, see Figure \ref{fig:ex1} (d))}, then $b(H_i)$ is orthogonal to some vector $\hat{e}$ in $cone(H_i)$. This guarantees that the base is placed on the 3D printing platform.
...
\ENSURE~
The decomposition of $M$ into the least pieces of components,  

a set of subgraphs $T$ that satisfies constraint (1), and 
...
\STATE $S = S / H$;
\STATE $U = U \cup H$;
\STATE $(M^', N) = MeshDecomposition(U , M)$;
\IF {$N < min$}
\STATE $T = U$;
\STATE  $min = | U | $;
\ENDIF
