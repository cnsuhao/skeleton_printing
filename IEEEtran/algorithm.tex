...
in order to guarantee support-free fabrication for the boundary of the cut, the angle between $n(c)$ and $r(H_i)$ should satisfy: $A(n(c), r(Hi)) \leq \pi/2 + \theta -\alpha $, where $A(\cdot)$ denotes angle \youyi{confusing, what does pointing to the exterior of the partitioned part mean? double check this constraint, i think it is related to both the angle of the cone and $\theta$};
