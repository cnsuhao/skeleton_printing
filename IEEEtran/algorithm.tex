...
Next we shall show how $Trim\_BFS(v, G, \theta)$ works to find a locally maximal subgraph starting at $v$ that satisfies the angle constraint. Let $H$ be the current subgraph obtained so far, initially $H$ is a single directed arc. When an arc $e$ of $G$ is visited, we need to determine whether it should be included into $H$. If the start of each outgoing arc of $H$ is moved to a common origin, then the arcs form a fork of rays (Figure \ref{fig:sphere}). A naive method for judging whether $e$ should be included is to move the start of $e$ to the origin of the fork, and compute the angle between $e$ and each arc of the fork, $e$ is included if the maximum angle between $e$ and each arc of the fork does not exceed 2$\theta$. However, this would lead to $O(n^{2})$ time complexity of $Trim\_BFS(v, G, \theta)$, where $n$ is the size of $G$. To speed up this process, we apply Preparata's real-time algorithm for computing the most-distant pair of vectors as follows: Consider unit sphere that is centered at the root of the fork, uniform the magnitude of each vector as $1$, then each vector leaves its endpoint
 on the sphere. Now, consider the ($2D$) spherical convex hull of these endpoints in which the arc between any air of points is the geodesic shortest path between them on the sphere. The diameter of the spherical convex hull is defined as the longest spherical arc between a pair of points on the sphere. Under this definition, by changing the Euclidean distance into geodesic distance, we can apply Preparata's algorithm for updating the spherical convex hull, and the diameter of the spherical hull can be determined in the process of computing the tangents of a point that is external to the current hull. Each update of the diameter can be done in $O(log n)$ time in the worst case by using the $AVL$ tree data structure.
 
 Thereafter, as a new ray $e$ is visited, we compute the acute angle between $e$ and $Sum(H)/num(H)$, if it is no larger than $\theta$, then it is safe to take it into $H$, else otherwise. Clearly, this takes O(1) time in considering an edge.

% F. P. Preparata. An optimal real-time algorithm for planar convex hulls. Comm. ACM, 22(7):402¨C405, 1979.
