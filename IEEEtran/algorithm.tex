...
Therefore, let $b(H_i)$ be the base cut of the mesh part that corresponds to the root of $H_i$ \ca{(by base we mean the cutting cross section that touches the printing platform when the part is being printed, see Figure \ref{})}, then $b(H_i)$ is orthogonal to some vector $\hat{e}$ in $cone(H_i)$. This guarantees that the base is placed on the 3D printing platform.
