...
\item (ii) The base of a partitioned model corresponding to a subgraph $H_i$ is orthogonal to some vector in $cone(H_i)$, where $cone(H_i)$ is the cone that encapsulate all valid support-free printing directions in accordance to arcs in $H_i$ when printed (see Figure \ref{fig:cone}).
