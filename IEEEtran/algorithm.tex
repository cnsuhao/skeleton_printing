...
\begin{figure}[tbp]
  ...
  \caption{\label{fig:meshpartition}%
           {\color{blue}2D illustration of partition the mesh for different scenarios of $F(H_i)$ and $F(H_j)$. (a)$F(H_i) \cap F(H_j) \neq \emptyset$, a cut $c$ that realizes the minimum peripheral length is induced; (b)$F(H_i) \cap F(H_j) = \emptyset$, two cuts $c_1$ and $c_2$ are required; (c)a cut $c$ along the angular bisector of $c_1$ and $c_2$ is induced when $A(c_1, c_2) \leq \pi/2+\theta$.} \mcc{[Youyi: what is $C_3$ in (c)?]} }
\end{figure}
