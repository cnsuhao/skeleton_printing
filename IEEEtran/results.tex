...
\hl {[Evaluations: how close we are to the global optimum][Limitations: discussion about 1. the strength of our interior, -- future work. 2. cut through salient regions -- hard to balance. etc]} \youyi{We need some efforts in the paragraph below.----------I have moved related wordings into the conclusion section.}

%Since the problem of partitioning the skeleton graph into the least number of support-free subgraphs is NP-hard,
Since the partition of $S$ may have exponential amount of choices, it is impossible to obtain an optimal mesh partition in polynomial time. Particularly, which arcs to be taken into a subgraph and the taking sequence significantly affect the partition result. However, from Table \ref{tab:ertms:summary}, we can conclude that the partition numbers for the skeletons and mesh models are small enough. Further, we can evaluate our partition result by examining some simple examples.

In practice, most natural mesh models are round in the sense that there is no flat platform for supporting it on the ground, and therefore at least one cut is required. Refer to the  tree-liked shell model in Figure \ref{fig:simpletree}, our algorithm partitions the model into two parts (shown in red and blue respectively) with one cut, and is therefore optimal.

\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.2\textwidth]{figs/simpletree.png}
  \caption{\label{fig:simpletree}%
           An illustration of a simplified tree model that is partitioned into the least pieces by our algorithm.}
\end{figure}
